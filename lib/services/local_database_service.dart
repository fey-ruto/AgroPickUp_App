  ...
  Future<Database> _initDb() async {
    ...
    return openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 3) {
          await db.execute(
              'ALTER TABLE pickup_requests ADD COLUMN aggregatorId TEXT');
          await db.execute(
              'ALTER TABLE pickup_requests ADD COLUMN aggregatorName TEXT');
          await db.execute(
              'ALTER TABLE collection_points ADD COLUMN ownerAdminId TEXT');
          await db.execute(
              'ALTER TABLE collection_points ADD COLUMN ownerAdminName TEXT');
        }
      },
    );
  }
  ...
  Future<void> cacheCollectionPoint(CollectionPoint point) async {
    ...
    await db.insert(
      'collection_points',
      {
        ...
        'region': point.region,
        'ownerAdminId': point.ownerAdminId,
        'ownerAdminName': point.ownerAdminName,
  ...
  Future<List<CollectionPoint>> getCachedCollectionPoints() async {
    ...
    return maps
        .map((m) => CollectionPoint(
              ...
              ownerAdminId: (m['ownerAdminId'] as String?) ?? '',
              ownerAdminName: m['ownerAdminName'] as String?,
